...
const ProfileQrCode = ({ data }) => {
    ...
    return (
        <Fragment>
            <Card className="profile-qrcode text-center " style={{ backgroundImage: `url(${source})` }}>
                <CardBody className="px-0">
                    <div className="logo-mask" >
                        <ImageRounded src={LogoKejaksaan} width={45} height={44} />
                    </div>

                    <div className="card-name">
                        <small>KEJAKSAAN AGUNG RI</small>
                        <p style={{ fontSize: "14px", marginBottom: "0px" }}>{data.name}</p>
                        <small>NIP : {data.identity_id}</small>
                    </div>
                    <div className="mt-1">
                    ...
                    <div style={{ marginTop: "20px" }}>
                        <span className="mt-4">{data.workunit}</span><br />
                        <span className="font-weight-bolder">{data.sector}</span>
                    </div>
