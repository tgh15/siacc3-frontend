...
import ChatMonitoring from "../utils/chat-monitoring";
...
const MonitoringBotMessage = () => {
    ...
    const updateData = (data) => {
        setIsLoading(true);
        HelpdeskChatbotApi.update(data)
            .then(res => {
                setIsLoading(false);
                getData();
    ...
    const deleteData = (id) => {
        setIsLoading(true);
        HelpdeskChatbotApi.remove(id).then(res => {
            setIsLoading(false);
            getData();
        }
            , err => {
    ...
    const getData = () => {
        setIsLoading(true)
        HelpdeskChatbotApi.getAll(params.current).then(res => {
            setIsLoading(false)
    ...
    const getDetail = (id) => {
        ...
        HelpdeskChatbotApi.open(id).then(res => {
        ...
        <Row>
            <Col md={8}>
                {!loadingMessage ? dataSelected != null ?
                    <ChatMonitoring data={dataSelected} />
                    : <EmptyChat />
                    :
                    <Skeleton style={{
                        width: "100%",
                                ...
                                datas.length > 0 ?
                                    datas.map((data, index) => (
                                        <CardItem
                                            ...
                                            image={data.questioner?.photo}
                                            title={`${data.questioner?.name} -  ${data.questioner?.position}`}
...
export default MonitoringBotMessage;