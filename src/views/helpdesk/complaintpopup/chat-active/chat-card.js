...
const ChatCard = props => {
    ...
    const { messages, uuid, scrollToBottom, setImageSelected } = props;
        ...
        return dataMessage.length > 0 ? (
            <div
                ...
                className={classNames('chat', {
                    'chat-left': item.senderId == "" || item.senderId != uuid
                })}
            >

                <div className='chat-avatar'>
                    {item.senderId != '' && item.senderId == uuid ? <span className="mr-1 fw-bolder"> {item.senderName} </span> : null}
                    <Avatar
                    ...
                    {item.senderId == '' ? <span className="ml-1"> Sibot SIACC </span> : null}
                    {item.senderId != '' && item.senderId != uuid ? <span className="ml-1"> {item.senderName} </span> : null}
                </div>
